...
init_state = np.zeros((len(czbins), len(ebins), 2, 4))
...
if not os.path.exists("./standard.h5"):
    nuSQ = nsq.nuSQUIDSAtm(czbins, ebins, 4, nsq.NeutrinoType.both, True)
    setup_nsq(nuSQ)
    nuSQ.EvolveState()
    nuSQ.WriteStateHDF5("./standard.h5", True)

standard_nusq = nsq.nuSQUIDSAtm("./standard.h5")
...
fast = nsq.nuSQUIDSAtm(czbins, ebins, 4, nsq.NeutrinoType.both, True)
setup_nsq(fast)
fast.Set_AutoEvolLowPass(np.pi/f, np.pi/fp, True)
fast.EvolveState()

before = nsq.nuSQUIDSAtm(czbins, ebins, 4, nsq.NeutrinoType.both, True)
setup_nsq(before)
