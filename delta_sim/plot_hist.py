    ...
    for c_i,(energy,l_energy) in enumerate(zip(energies, e_string_list)):
        ...
        for key in particle_types.keys():
            if key not in d_dict[l_energy]:
                continue
            loss_hists_by_primary[energy][key] = np.cumsum(np.array(d_dict[l_energy][key]), axis=1)
            max_entry = max(max_entry, np.amax(loss_hists_by_primary[energy][key]))
...
for i, pos in enumerate(pos_bins[1:]):
    if pos < 10:
        continue
    for keys,key_label in zip(keys_to_plot,labels):
        ...
        for c_i,energy in enumerate(energies):
            ...
            ax.set_xlabel(r"Ionization energy [GeV]")
            ax.set_ylabel(r"Avg. losses per muon in %d m" % pos)
            #ax.set_title(key_label)
            ax.set_xscale("log")
            ax.set_ylim((0, 0.4))
            ax.legend()
            fig.tight_layout()
            fig.savefig(args.output + "_" + key_label + ("_%02dm"%pos) + ".png", dpi=200)
        ax.cla()
