...
default_flux_path = '/n/holyscratch01/arguelles_delgado_lab/Lab/DUNEAnalysis/store/fluxes_lowpass/'
...
#f1_3_grid = np.linspace(-1, 1,10+1)
#f2_3_grid = np.linspace(0, 1, 5+1)
...
#f1_4_grid = np.linspace(-1, 1, 10+1)
#f2_4_grid = np.linspace(0, 1, 5+1)

flux = nuflux.makeFlux("H3a_SIBYLL23C")
osc = oscillator.oscillator(
    "H3a_SIBYLL23C", flux, ebins, czbins, "lv", flux_path, cache_size=10
)
osc_init = oscillator.oscillator(
    "H3a_SIBYLL23C", flux, ebins, czbins, "init", flux_path, cache_size=1
)
osc_init[tuple()]
del osc_init
