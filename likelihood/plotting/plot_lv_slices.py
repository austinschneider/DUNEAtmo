...
fnames = glob.glob(
    "/n/holyscratch01/arguelles_delgado_lab/Lab/DUNEAnalysis/store/scans/lv/small/*.json"
)
...
for json_fname in fnames:
    ...
    for line in f:
        ...
        for entry in json_data:
            ...
            index_by_params[
                (
                    int(entry["operator_dimension"]),
                    float(entry["lv_mutau_re"]),
                    float(entry["lv_mutau_im"]),
                    float(entry["lv_mumu"]),
                )
            ] = 1
            i += 1
...
null_index = index_by_params[(3, 0.0, 0.0, 0.0)]
...
for dim in [3, 4]:
    ...
    lv_mumu = val_by_key["lv_mumu"]


    mask = functools.reduce(
        np.logical_and,
        [val_by_key["operator_dimension"] == dim, lv_mutau_re > 0, lv_mutau_im > 0],
    )
    unique_lv_mumu = sorted(np.unique(lv_mumu[mask]))

    for mumu in unique_lv_mumu:
        this_mask = np.logical_and(mask, lv_mumu == mumu)

        lv_mutau_re = val_by_key["lv_mutau_re"][this_mask]
        lv_mutau_im = val_by_key["lv_mutau_im"][this_mask]
        llh = val_by_key["llh"][this_mask]
        ...
        if power == 0:
            ...
            if power == 1:
                ...
                units = r"[GeV^" + str(power) + r"]"

        #ax.set_title(r"$%.3e$" % mumu)
        ax.set_title(r"$%.3e$" % mumu)
        ...
        ax.legend()
        #fig.tight_layout()
        fig.savefig(outdir + "lv_dim" + ("%d" % dim) + "_" + (r"%.3e" % mumu) + "_sensitivity.pdf")
        fig.savefig(outdir + "lv_dim" + ("%d" % dim) + "_" + (r"%.3e" % mumu) + "_sensitivity.png", dpi=200)
