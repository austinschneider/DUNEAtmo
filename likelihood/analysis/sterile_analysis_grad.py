...
def setup_sterile_analysis():
    ...
    def load_mc():
        ...
        mc = data_loader.load_data(glob.glob(weight_path + "weighted_*.json"))
        tg_emin = 1e2,
        tg_emax = 1e5,
        start_emin = 1e2,
        start_emax = 1e5,
    ...
    def conv_flux_tilt_corrected(baseline_flux_conv, conv_tilt_correction):
        conv_tilt_correction, conv_tilt_correction_d = conv_tilt_correction
        res = baseline_flux_conv * conv_tilt_correction
        res_d = baseline_flux_conv * conv_tilt_correction_d
    ...
    def flux_conv(convNorm, conv_flux_tilt_corrected):
        conv_flux_tilt_corrected, conv_flux_tilt_corrected_d = conv_flux_tilt_corrected
        res = convNorm * conv_flux_tilt_corrected
        res_d_cr = convNorm * conv_flux_tilt_corrected_d
        res_d_norm = conv_flux_tilt_corrected
        assert(not np.any(np.isnan(res)))
...
if __name__ == "__main__":
    ...
    def asimov_binned_likelihood(parameters, asimov_parameters):
        ...
        return likelihood_grad.LEff(ad.unpack(asimov_expect)[0], ad.unpack(expect), ad.unpack(expect_sq))
